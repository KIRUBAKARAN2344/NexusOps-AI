...
3. Your output MUST be in valid JSON format matching this schema:
{
  "recommended_action": "string (the specific action to take from the runbook)",
  "confidence": "float (0.0 to 1.0)",
  "evidence_citations": ["string (exact clauses from the runbook or specific alert IDs that led to this)"],
    ...
    def generate_recommendation(self, incident: Incident, runbook: Runbook) -> Recommendation:
        ...
        if reason:
            return Recommendation(
                ...
                recommended_action="Escalate to Human (Rule-based).",
                confidence=1.0,
                evidence_citations=[],
        ...
        if not runbook:
            return Recommendation(
                incident_id=incident.incident_id,
                recommended_action="Escalate to Human. No matching runbook found.",
                confidence=1.0,
                evidence_citations=[],
                runbook_id=None,
                requires_escalation=True,
